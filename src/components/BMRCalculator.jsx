...
export default function BMRCalculator({ onBack, onBMRUpdate }) {
  ...
  const calculateBMR = async () => {
    ...
    if (!weight || !height || !age || weight <= 20 || weight > 300 || height <= 50 || height > 250 || age < 12 || age > 120) {
    ...
    const gainCalories = maintenanceCalories + 250;
    const loseCalories = maintenanceCalories - 250;
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/auth/updateBMR`, {
        ...
        body: JSON.stringify({
          bmr: maintenanceCalories,
          maintenance: maintenanceCalories,
          gainWeight: gainCalories,
          loseWeight: loseCalories,
        }),
      });
      console.log(gainWeight);

      if (!response.ok) throw new Error("Failed to update BMR in profile");

      console.log("✅ BMR updated successfully");

      if (onBMRUpdate) onBMRUpdate();
    } catch (error) {
      console.error("❌ Error updating BMR:", error.message);
    }
  };
  ...
  return (
    <div className="relative flex w-full">
      ...
      <div className="w-[300px] p-6 ml-4 bg-gray-100 rounded-2xl shadow-lg flex flex-col items-center">
        <h2 className="text-xl font-bold text-gray-800 text-center">Your BMR Results</h2>
        {calories ? (
          <div className="text-center mt-4">
            <p className="text-2xl font-semibold text-gray-700">Maintenance Calories: {calories} kcal</p>
            <p className="text-xl text-green-600 mt-2">To Gain Weight: {calories + 250} kcal</p>
            <p className="text-xl text-red-600 mt-2">To Lose Weight: {calories - 250} kcal</p>
          </div>
        ) : (
